...
public class PageRankDriver {
	...
	static void init(String input, String output, int reducers)
		...
		System.out.println("$ Tiankai Xie (10409777)");
	...
	static void nameJoin(String input, String nameInput, String output, int reducers) throws  Exception{
		System.out.println("$ Tiankai Xie (10409777)");
		System.out.println("$ NameJoin Job 1 Started ");
		Job job = Job.getInstance(); // Creates a new Job
		job.setJarByClass(PageRankDriver.class); // Sets the Driver class
		job.setNumReduceTasks(reducers); // Sets the number of reducers

		FileInputFormat.addInputPath(job, new Path(input)); // Adds input and output paths
		FileInputFormat.addInputPath(job, new Path(nameInput));
		FileOutputFormat.setOutputPath(job, new Path("tempjoin"));

		job.setMapperClass(NameMapper.class); // Sets Mapper and Reducer Classes
		...
		if (job.waitForCompletion(true)){
			System.out.println("$ Tiankai Xie (10409777)");
			System.out.println("$ NameJoin Job 1 Finished, NameJoin job 2 Started ");
			Job job1 = Job.getInstance(); // Creates a new Job
			job1.setJarByClass(PageRankDriver.class); // Sets the Driver class
			job1.setNumReduceTasks(reducers); // Sets the number of reducers
			job1.setSortComparatorClass(LongWritable.DecreasingComparator.class);

			FileInputFormat.addInputPath(job1, new Path("tempjoin")); // Adds input and output paths
			FileOutputFormat.setOutputPath(job1, new Path(output));

			job1.setMapperClass(NameMapper2.class); // Sets Mapper and Reducer Classes
			job1.setReducerClass(NameReducer2.class);
			...
			job1.setOutputValueClass(Text.class);
			// Prints message on successful completion or error.
			System.out.println(job1.waitForCompletion(true) ? "$ NameJoin Job 2 Completed" : "$ NameJoin Job 2 Error");
			// Exits once job finishes.
	...
	public static void composite(String input, String output, String interim1,
		...
		int counter = 0;
		init(input+"/links", interim1, reducers); // Initializes data
		counter++;
		...
		while (difference >= 30) // continues operation till difference is of specified value
		{
			...
			if (i % 3 == 0) // for every 3 iterations calculates the difference
			{
				 ...
				 */
				difference = readDiffResult(diff); // Reads difference from "diffout"
				System.out.println("$ Difference updates to:" + difference);
				deleteDirectory(diff); // deletes diffout
			}
						...
						// is the input directory
		{
			...
			counter++;
			finish(interim2, "res1", reducers);
		} else // for even i, interim1 is the input directory
		{
			deleteDirectory(interim2); // Deletes other directory
			counter++;
			finish(interim1, "res1", reducers);

		}
		nameJoin("res1",input+"/names",output,reducers);
		...
		if (fs.exists(diffpath)) {
			...
			for (FileStatus file : ls) { // Modified file to support multiple reducer output files.
				if (file.getPath().getName().startsWith("part-r-00")) {
					...
					if (diffcontent != null) {
						if (diffcontent.contains("maxdif")){
							diffcontent = diffcontent.substring(7);
						}
